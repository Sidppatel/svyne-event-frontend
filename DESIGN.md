# TicketSpan "Box Office" Design System

**The single source of truth** for UI/UX, themes, shadcn components, and animation across the TicketSpan event platform. If a styling decision isn't here, it isn't a standard. Tokens live in `src/index.css` — this document mirrors and explains them.
...
<div className="overflow-hidden rounded-lg border border-border bg-card">
  ...
  <div className="ticketspan-ticket-edge" style={{ ['--ticketspan-notch' as string]: '#FBF6F0' }} />
  <div className="p-4 font-mono text-sm">{/* booking code, the stub */}</div>
</div>
```

`.ticketspan-ticket-edge` (in `index.css`) draws the dashed marigold line + end notches. The notch fill defaults to the page canvas; set `--ticketspan-notch` to the surface color when the edge sits inside a non-canvas element (e.g. `#ffffff` inside a white card). Use it on **event cards, the checkout order summary, and the My Bookings ticket.**
...
- **List stagger** — event/ticket grids fade up with a small per-item delay (CSS `animationDelay` on `.ticketspan-page`, or GSAP stagger).
- **Sticky buy-bar** — slides up on mount (`ticketspan-slide-*` curve `cubic-bezier(0.32,0.72,0,1)`).
- **Urgency** — `.ticketspan-urgent` (marigold pulse) **only** on "selling fast" / "limited" — never ambient.
