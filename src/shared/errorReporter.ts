...
const OFFLINE_STORAGE_KEY = 'ticketspan_pending_error_reports';
...
function sessionId(): string {
  let id = sessionStorage.getItem('ticketspan_error_session_id');
  if (!id) {
    id = crypto.randomUUID();
    sessionStorage.setItem('ticketspan_error_session_id', id);
  }
  return id;
}
