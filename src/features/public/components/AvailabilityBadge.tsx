...
export function AvailabilityBadge({ quantityLeft, maxQuantity, className }: AvailabilityBadgeProps) {
  ...
  if (left !== undefined && left <= 10) {
    return (
      <span className={cn('inline-flex items-center gap-1.5 rounded-full bg-accent-gold/15 border border-accent-gold/20 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-accent-gold ticketspan-urgent', className)}>
        Only {left} left
      </span>
    );
  }
