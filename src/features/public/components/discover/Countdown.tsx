...
function Unit({ value, label }: { value: string; label: string }) {
  return (
    <div className="flex flex-col items-center">
      <span key={value} className="ticketspan-page font-mono text-2xl font-medium tabular-nums text-on-stage md:text-3xl">
        {value}
      </span>
  ...
  if (parts.live) {
    return (
      <span className="inline-flex items-center gap-2 rounded-full bg-voltage/20 px-3 py-1 font-mono text-xs font-medium uppercase tracking-widest text-on-stage">
        <span className="size-2 rounded-full bg-voltage ticketspan-urgent" />
        Happening now
      </span>
    );
  }
