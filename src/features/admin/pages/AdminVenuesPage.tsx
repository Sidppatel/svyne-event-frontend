      ...
      <div className="border border-border bg-card shadow-sm rounded-2xl overflow-hidden transition-all duration-300">
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-end gap-4 border-t border-border/20 pt-4">
            <Button onClick={add} disabled={!draft.name.trim()} className={cn("svyne-spring-btn h-11 px-8 rounded-xl font-bold uppercase tracking-wider text-xs shadow-md shadow-primary/20", !draft.name.trim() && "opacity-40 cursor-not-allowed")}>
              Add venue
            </Button>
          </div>
        </CardContent>
        ...
        <div className={cn(
          ...
          <div className="overflow-hidden space-y-4 pt-2.5">
            ...
            <div className="flex items-center justify-end border-t border-border/10 pt-3">
              <Button size="sm" onClick={() => persist(venue.isActive)} className="svyne-spring-btn h-9 px-6 rounded-lg font-bold text-xs">
                Save Settings
              </Button>
            </div>
